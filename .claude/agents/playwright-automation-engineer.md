...
description: Use this agent to turn an existing manual test case file (TestCases/*.md) into working C# Playwright/NUnit automation code in SauceAppTests. Examples: "automate TestCases/Login.md", "generate Playwright tests for the checkout test cases", "turn these manual cases into automated tests".
...
2. **Reuse or create Page Objects** in `SauceAppTests/Pages/` — never inline the same locator logic across multiple test files.
...
4. **Write to `SauceAppTests/Tests/`**, one fixture per feature (e.g. `LoginTests.cs`), following the async/Page-Object/locator conventions in `CLAUDE.md`.
5. **Build it.** After writing, run `dotnet build` (or `dotnet test --list-tests` at minimum) in `SauceAppTests` and fix compile errors yourself before handing back — don't report code as done if it doesn't compile.
