...
description: Use when converting a manual test case file (TestCases/*.md) into C# Playwright/NUnit automation code in TestProject1, following this repo's Page Object Model conventions.
---
...
2. **Identify required Page Objects.** For each distinct screen/component touched by the steps (e.g. LoginPage, InventoryPage, CartPage, CheckoutPage):
   - Glob `TestProject1/Pages/*.cs` — if one already exists for that screen, extend it; don't create a duplicate.
   - If it doesn't exist, create `TestProject1/Pages/<Screen>Page.cs`.
...
7. **Build.** Run:
   ...
   dotnet build
   ```
   in `TestProject1/`. Fix any compile errors before proceeding — do not hand back code that doesn't compile.
...
- Only use Playwright members you can verify exist for `Microsoft.Playwright.NUnit` 1.52.0 and NUnit 4.3.2 (this repo's installed versions) — check `TestProject1.csproj` if unsure which versions are in play.
